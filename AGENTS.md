...
- LSP: Neovim-native `vim.lsp.config` / `vim.lsp.enable`. The legacy `require('lspconfig').setup{}` framework is deprecated and must not be used. Servers are installed and enabled through `mason-org/mason` and `mason-lspconfig` (v2, with `automatic_enable` excluding `ruff` and `stylua`, which are installed as formatters via mason-tool-installer, not language servers). Per-server overrides go in `after/lsp/<server>.lua`, which core merges over the defaults. Note the org move: it is `mason-org/*` now, not the old `williamboman/*`.
